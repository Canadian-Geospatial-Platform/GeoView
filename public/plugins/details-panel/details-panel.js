    ...
    class DetailsPanel {
        ...
        translations = {
            'en-CA': {
                ...
                action_back: 'Back',
                activate_crosshair: 'Activate Crosshair',
            },
            'fr-CA': {
                detailsPanel: 'Détails',
                nothing_found: 'Aucun résultat',
                action_back: 'Retour',
                activate_crosshair: 'Activer le réticule',
            },
        };
        ...
        added = () => {
            ...
            const Component = () => {
                ...
                useEffect(() => {
                    ...
                    // create a navbar button that would trigger a crosshair
                    api.map(mapId).createNavbarButton({
                        id: 'crosshair',
                        tooltip: t('activate_crosshair'),
                        icon: `
                            <svg
                        ...
                        callback: () => {
                            ...
                            this.buttonPanel.panel.close();

                            // trigger focus event
                            api.event.emit('map/crosshair_enable_disable', mapId, {
                                buttonId: this.buttonPanel.id,
                            });
                    ...
                    mapLayers.forEach(async (mapLayer) => {
                        ...
                        if (mapLayer.type === 'ogcWMS') {
                            // query the layer information, WMS server URL will always query 0
                            const layerInfo = await queryServer(mapLayer.layer.mapService.options.url + 0);

                            // try to add the legend image url for the WMS layer
                            const legendImageUrl = `${mapLayer.layer._url}?request=GetLegendGraphic&version=1.0.0&Service=WMS&format=image/png&layer=0`;
