...
export class Vector {
     ...
     *
     * @returns a geometry containing the an id and the created layer
     */
    addPolyline = (
        ...
        const lId = id !== null && id !== undefined && id.length > 0 ? id : this.generateGeometryId();

        // create polyline
        const polyline = this.polyline.createPolyline(lId, startPoint, endPoint, options);
    ...
    addPolylineCustom = (
        ...
        const polyline = this.polyline.createCustomPolyline(lId, points, options);
