...
/**
 * Class to handle configuration validation. Will validate every item for sstructure and valid values. If error found, will replace by default values
 * and sent a message in the console for a developers to know something is wrong
 *
 * @exports
 * @class
 */
export class Config {
     ...
     * @param {JSON} confif JSON configuration object
     * @returns {MapConfigProps} valid JJSON configuration object
     */
    private validate(id: string, config: string): MapConfigProps {
        ...
        // recreate the prop object to remove unwanted items and check if same as original. Log the errors
        const validConfig: MapConfigProps = { id, projection, zoom, center, language, basemapOptions };
        this.logErrors(tmpConfig, validConfig);

        return validConfig;
    }

    /**
     * Log error and modification made to configuration by the validator
     * @param {MapConfigProps} inConfig input config
     * @param {MapConfigProps} validConfig valid config
     */
    private logErrors(inConfig: MapConfigProps, validConfig: MapConfigProps): void {
